...
i=1;
imageFileName = strcat('c', int2str(i), '0.jpg'); %X
outputFileName = '29072017X-test.txt'; %X
pixelToMMRatioInX = 10 ./ 86;
pixelToMMRatioInY = 10 ./ 140; % 29.07
...
isItXaxis = false; %X
imageInspection_ON = false; %X
dataInspection_ON = false; %X
makeFit_ON =  true;%X
...
if makeFit_ON == true
	...
	if(saveFitResults == true)
		...
		magneticFieldStr = magneticFields(i); %X
		fprintf(pFile, '\n%s %f %.5f %d %d %d %d %d %d %d', imageFileName, sigma, magneticFieldStr, numberOfCutsToMake, dataIndexBeg, dataIndexEnd, firstCutBeg, firstCutEnd, secondCutBeg, secondCutEnd);
	end
